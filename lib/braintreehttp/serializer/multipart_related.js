...
class MultipartRelated {
  ...
  _formPart(key, formPart, boundary, request) {
    ...
    if (request.partHeaders && request.partHeaders[key]) {
      let partHeaders = request.partHeaders[key];
      formPartContentType = partHeaders['Content-Type'];
    }

    let contentBuffer = null;
    if (formPartContentType === 'application/json') {
      contentBuffer = Buffer.from(JSON.stringify(formPart) + MultipartRelated._CRLF)
    } else {
...
module.exports.MultipartRelated = MultipartRelated;
